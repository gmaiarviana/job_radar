...
📡 **Status:** Pipeline completo (fetch → filter → score → dashboard). Épicos 8, 9 e 10 concluídos. Próximo foco: Épicos 7.3 (Expansão de Coleta) e 11 (UX Completa).
...
**Revisão (estado do código):** Conferido em Mar 2026. Concluídos conforme seção ✅ (incluindo Épicos 9 e 10); `github_api.py` existe e está integrado ao app. Épicos 11 (feedback/histórico + marcação "Já apliquei"), 12 (retry/tratamento de falhas), 13 (generate.py completo) e 14 ainda não implementados. `generate.py` segue stub.
